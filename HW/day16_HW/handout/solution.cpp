...
bool move_left(std::vector<std::vector<int>>& board) {
    ...
    for (int i = 0; i < 4; i++) {
        std::vector<int> original = board[i];

        std::vector<int> row = compress_row(board[i]);
        row = merge_row(row);

        if (row != original) moved = true;
...
bool move_up(std::vector<std::vector<int>>& board) {
    ...
    for (int c = 0; c < 4; c++) {
        std::vector<int> original(4);
        std::vector<int> col(4);

        for (int r = 0; r < 4; r++) {
            original[r] = board[r][c];
            col[r] = board[r][c];
        }
...
bool move_down(std::vector<std::vector<int>>& board) {
    ...
    for (int c = 0; c < 4; c++) {
        std::vector<int> col, original;

        for (int r = 0; r < 4; r++) {
            col.push_back(board[r][c]);
            original.push_back(board[r][c]);
        }
